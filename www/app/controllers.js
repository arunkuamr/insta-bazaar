angular.module('app.controllers', [])
  ...
  .controller('loginCtrl', function ($scope, $rootScope, $ionicHistory, sharedUtils, $state, $ionicSideMenuDelegate, authService) {
    ...
    $scope.loginEmail = function (formName, cred) {
      ...
      if (formName.$valid) { // Check if the form data is valid or not
        ...
        var value = authService.loginEmail(cred);
        if (value) {
          sharedUtils.hideLoading();
          $state.go('signup', {}, {
            location: "replace"
          ...
          sharedUtils.hideLoading();
          sharedUtils.showAlert("Email address already exist");
        }
  ...
  .controller('signupCtrl', function ($scope, $rootScope, sharedUtils, $ionicSideMenuDelegate,
    $state, $cordovaCamera, fireBaseData, $ionicHistory, authService) {
    ...
    $scope.singUpStep = function () {
      var userDetail = $scope.singUp.regUser;
      if (userDetail.email) {

        if ($scope.singUp.step == 1) {
          var value = authService.loginEmail(userDetail);
          if (value == false) {
            $scope.singUp.step += 1;
          } else {
            sharedUtils.showAlert("Email address already exist you can not create account");
          }
        } else if ($scope.singUp.step == 2) {
          if (userDetail.businessName && userDetail.businessDesc && userDetail.businesstype && userDetail.phoneNo && userDetail.address) {
            $scope.singUp.step += 1;
          } else {
            sharedUtils.showAlert("Fields should not be empty");
          }

        } else if ($scope.singUp.step == 3) {
          if (userDetail.pinCode && userDetail.gender) {
            $state.go('landing');
          } else {
            sharedUtils.showAlert("Fields should not be empty");
          }
